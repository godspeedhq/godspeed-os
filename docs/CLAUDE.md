...
| `observability.md`  | **Design note:** an observability service for metrics, logs and traces, and why it needs NO kernel endpoint. Traces are already service-side with zero kernel growth (the `trace` ring lives in `logger`); logs ride the EXISTING 11.4 kernel floor - `ctx.log()` is syscall 5 straight to the ring + serial, NOT IPC to the logger, so logging survives the logger's death and must NOT be re-pointed at `events`; kernel state is already exposed by PULL (`InspectKernel`, 24 queries, gated by INTROSPECT). The real gap is EVENTS a poll cannot see - and a push endpoint fails there exactly as a kernel trace ring would (a queue needs a drop policy, and deciding what to discard is judgement = policy). The shape that works is 11.4's: a bounded ring the kernel writes and a service DRAINS. Includes the three-part test any new kernel observability mechanism must pass, the `logger` -> `events` rename, and a PHASED PLAN: build the service against what exists (zero kernel change), find out what is genuinely missing (predicted: capability denials, which are never logged), and only then add a typed event ring - with the fast-path, no-strings and frozen-ABI rules it must obey |
| `logging.md`        | **Logging (design, not built):** what the `logger` service is *for* - a stateless **broker, not a store**. Logging is a *pipe* (no ambient stdout, so it is the same primitive as `cmd1 \| cmd2`); reading a service's stream is a **delegated resource capability** (§7.10) rather than a filter over one merged stream, so who-hears-whom is kernel-enforced and revocable; attribution is unforgeable (the kernel already knows the caller). Why stateless is load-bearing (a persisting logger cycles through `fs`, and worse, makes observing a storage failure depend on storage). The serial + ring-buffer floor, the loss-policy and no-heap obligations, and what the 29-line stub actually does today |
