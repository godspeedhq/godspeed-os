...
| `gsfs-carnage.md`   | **Scoped, not built:** maximum carnage for GSFS. The invariant (*a failed operation must never be reported as successful, and recovery must never silently turn known corruption into apparently valid data*), the PERMITTED-OUTCOME TABLE per operation - which is the deliverable everything else depends on and had never been written down - the detection boundary (a torn BLOCK is caught by its CRC; a torn SEQUENCE cannot be and is the permitted outcome), and five workstreams: torn-write injection, kill-mid-transaction, cross-ISA interchange, a host-side differential reference model, and persistence measured in bytes |
