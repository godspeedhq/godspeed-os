...
delete from the directory side. The check is the free accounting: bare `drives` reports the
...
| Physical-hardware validation | **selfcheck: 5 of 5.** Power cut: **5 of 5, ALL UNASSISTED** | **`selfcheck` on all five boards** (3.12). **EVERY BOARD HAS NOW BEEN CUT WITH NO HELP AND EVERY ONE RECOVERED IN THE STRONG FORM** - the journal replayed, content intact, accounting correct without repair. Unassisted means a plain image, no `crash-window` build, the power pulled at a moment nobody chose: Dell Wyse 5070 (AHCI, 2026-09-18, mid-`selfcheck`, the fourth cut landing in the window), HP T630 (AHCI, 2026-09-22, first attempt), Raspberry Pi 2 (USB/dwc2, 2026-09-23, third), Raspberry Pi 4 (USB/xhci, 2026-09-23, first), VisionFive 2 (USB/xhci riscv64, 2026-09-23, first). Five boards, four ISAs, two storage stacks over three controllers. The Pi 4 and VisionFive had `crash-window` cuts first (2026-09-22); those are SUPERSEDED by the unassisted ones and no result here now depends on a held-open window. **What an unassisted hit buys** is the ordering itself: `journal recovered 4 block(s)` means the commit record was durable before any home block moved, enforced by the device rather than by ten idle seconds. **It is what retired 6.1's Pi 2 example** (amendment 2026-09-23; `backlog/42`). **ALL FIVE HAVE NOW BEEN CUT ON THE SAME, CURRENT BUILD** (2026-09-23), so the row is one code state rather than four. That build carries the loud recovery path, which matters for the one board that MISSED: the Wyse's clean mount is a definitive miss rather than a silence that could also have been a torn record discarded without a word. The two AHCI boards were re-cut last and the T630 hit again (2844 writes in 16 s, its storage the fastest in the fleet), re-earning its accounting clause on shipping code with the free count actually moving (250008536 -> 250008541) rather than resting on the previous day's run. **The `nothing repaired` clause IS earned on the other FOUR, the T630 included**: the FIRST VisionFive cut reported `REPAIRED the FREE COUNT ... off by 1`, traced to the mount reading the superblock BEFORE the replay that rewrites it. The fix landed 2026-09-22 13:01 (`9186d5ce`) and the T630 was cut at 16:12 the same afternoon, so its run carried it - and that run REPLAYED, which is the bug's own precondition, and still reported the count agreeing. Four boards have now had the superblock rewritten under them mid-mount and kept correct accounting. Only the Wyse has not, because only the Wyse has never had a cut that both landed in the window and moved the count. **And the hit rate was mis-stated twice before being measured**: an earlier revision put it under 2%, extrapolating `fs`'s own "under a millisecond" comment - written about an AHCI SSD - to USB. The window IS the checkpoint, and home writes are slow on a stick. Measured across all twelve unassisted cuts: **6 hits in 12**, so a board that misses twice is ordinary variance, not the 1-in-35 the bad figure implied. **The Wyse was re-cut on the current build (2026-09-23) and MISSED the window** (the T630, re-cut minutes later, HIT - two of two for that board) - a definitive miss rather than an ambiguous silence, since this build would have named a torn record had it found one, which is the first time an x86 clean mount has meant exactly one thing. That run moved the free count across the cut (62517834 -> 62517820) and a full tree rebuild still agreed, over the largest tree in the matrix (54 files, 3 dirs, 0 bad); but it does NOT earn the clause, because the stale-superblock bug only bites when the journal REPLAYS - the replay is what rewrites the superblock the mount already read. Earning it needs a cut that lands in the window AND shifts the count. The T630 additionally ran the EHCI BIOS handoff's hard case (`released=0 ... FORCED after timeout`) with the keyboard behind the hub still working; no other board has EHCI. |
| Kernel changes / scope boundary review | PASS | No kernel source change on this branch. `osdev build` runs 21 commandment checks and 75 redteam probes, including the kernel module set against 4.3 |
