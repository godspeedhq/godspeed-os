...
| Physical-hardware validation | **selfcheck: 5 of 5.** Power cut: **5 of 5** | **`selfcheck` on all five boards** (3.12). **THE POWER CUT IS 5 OF 5, every one in the STRONG form** - recovered, content intact, accounting correct WITHOUT repair. Dell Wyse 5070 (AHCI, 2026-09-18). VisionFive 2 (USB/xhci, riscv64) and Raspberry Pi 4 (USB/xhci, aarch64), both 2026-09-22, cut DETERMINISTICALLY with a `crash-window` build. **THREE of the five were then cut UNASSISTED** - a plain image, no held-open window, power pulled at a moment nobody chose - and all three landed inside the commit-to-checkpoint window: **HP T630** (AHCI, 2026-09-22, first attempt), **Raspberry Pi 2** (2026-09-23, third attempt - the board 6.1 named as the exception), **Raspberry Pi 4** (2026-09-23, first attempt, 16 s into `churn 30`, `selfcheck` 511/0/0). Each: `journal recovered 4 block(s)`, `churn verify` NONE torn, `drives check` 0 bad, and the superblock's free count matching a full tree rebuild with nothing repaired. That last clause is what an unassisted cut BUYS: a replay means the commit record was durable before any home block moved, so the ORDERING was enforced by the device rather than by ten idle seconds. **It is also what retired 6.1's Pi 2 example** (amendment 2026-09-23; `backlog/42`). **The `nothing repaired` clause was earned**: the FIRST VisionFive cut reported `REPAIRED the FREE COUNT ... off by 1`, traced to the mount reading the superblock BEFORE the replay that rewrites it. Fixed, then re-verified on three controllers. **And the odds were mis-stated here twice**: an earlier revision put an unaimed hit at under 2%, extrapolating `fs`'s own "under a millisecond" comment - which was written about an AHCI SSD - to USB. The window IS the checkpoint, and home writes are slow on a stick, so it is a large fraction of each transaction there. Measured: **three hits in five unassisted cuts**, which is why the Pi 2's two misses were ordinary variance rather than the 1-in-35 luck that number implied. The T630 also ran the EHCI BIOS handoff's hard case (`released=0 ... FORCED after timeout`) with the keyboard behind the hub still working; no other board has EHCI. |
