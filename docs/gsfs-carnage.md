...
| Physical-hardware validation | **selfcheck: 5 of 5.** Power cut: **2 of 5** | TWO DIFFERENT TESTS. **`selfcheck` passed on all five boards 2026-09-21** (3.12). **The POWER CUT is 2 of 5.** Dell Wyse 5070, 2026-09-18, AHCI SSD: a cut inside the commit-to-checkpoint window, `journal recovered 4 block(s)`, `churn verify` 6 files NONE torn, `drives check` 0 bad with nothing repaired. **VisionFive 2, 2026-09-22, USB mass storage** - the first non-AHCI backend cut, and DETERMINISTICALLY: a `crash-window` build holds the window open for ten seconds, so one cut lands inside it rather than one in four. `journal recovered 4 block(s)`, `churn verify` 6 files NONE torn, `drives check` 0 bad and NOTHING REPAIRED. **That last clause is what the second board bought**: the first VisionFive cut reported `REPAIRED the FREE COUNT ... off by 1`, which traced to the mount reading the superblock BEFORE the replay that rewrites it and then building memory from the stale copy. Fixed, and the strong form re-verified on the same board. The Wyse could not have shown it - its cut did not change the free count, so stale and fresh were identical. Three boards remain, and the Pi 2 is the one to watch: 6.1 records `fs` as restartable but NOT crash-recoverable there, because its stick refuses `SYNCHRONIZE CACHE`, and that claim has never met a machine. |
