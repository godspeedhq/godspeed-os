...
| Physical-hardware validation | **selfcheck: 5 of 5.** Power cut: **3 of 5** | TWO DIFFERENT TESTS. **`selfcheck` passed on all five boards 2026-09-21** (3.12). **The POWER CUT is 3 of 5**, all in the STRONG form - recovered, content intact, and the accounting correct WITHOUT repair. Dell Wyse 5070 (AHCI SSD, 2026-09-18). **VisionFive 2 (USB, riscv64, 2026-09-22)** - the first non-AHCI backend, cut DETERMINISTICALLY with a `crash-window` build that holds the window open ten seconds, so one cut lands inside it rather than one in four. **Raspberry Pi 4 (USB, aarch64, 2026-09-22)** - `selfcheck` 509/0/0 twice, `journal recovered 4 block(s)`, `churn verify` 6 files NONE torn, `drives check` 0 bad and nothing repaired. **The second and third boards are what earned the `nothing repaired` clause**: the FIRST VisionFive cut reported `REPAIRED the FREE COUNT ... off by 1`, traced to the mount reading the superblock BEFORE the replay that rewrites it. Fixed, then re-verified on both USB boards. The Wyse could not have shown it - its cut did not change the free count. Two boards remain (T630, Pi 2); `backlog/42` records that the Pi 2's stick now ATTESTS durability across three sessions while 6.1 says it refuses. |
