...
| **ARM (32-bit)** | `armv7a-none-eabi` | `-M raspi2b` + bare metal | firmware loads `kernel7.img` @ `0x8000` (HYP) | PL011 @ `0x3F20_1000` | **RUNS THE REAL OS BOOTSTRAP** - the kernel spawns the `supervisor`, which spawns the services (`logger`/`ping`/`pong`) from its manifest via the spawn syscall, and `ping`->`pong` capability-mediated IPC flows under preemptive scheduling (13028 messages, 0 faults on the Pi 2). Full machine layer + syscalls + user mode + scheduler + IPC + the neutral spawn path + supervisor | **Raspberry Pi 2 Model B v1.1** (2026-07-21) + QEMU |
