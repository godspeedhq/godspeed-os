...
- **Compile-bounded (all three):** `cargo check -p kernel --target <isa>` compiles with **0 errors** for
  AArch64 and RISC-V using only `arch/<isa>/mod.rs`; every neutral file - capability table, IPC,
...
- **Boot-bounded (all three):** each arch's `_start` + minimal boot brings the neutral kernel to life and
  drives its console. x86-64 goes all the way to the interactive shell; AArch64 and RISC-V reach the
...
- **Boot handoff:** x86 via Limine (higher-half, PHDRS); AArch64 via QEMU `-kernel` at `0x4008_0000` in
  EL1; RISC-V via OpenSBI (M-mode firmware) into S-mode at `0x8020_0000`.
- **First-instruction gotchas found by booting:** AArch64 traps FP/SIMD at EL1 by default - Rust emits
  ...
  `riscv64imac` (soft-float), sidestepping the FP-enable step entirely, and booted first try.
- **Linker scripts:** `kernel/kernel.ld` (x86, higher-half), `kernel-aarch64.ld` (virt `0x4008_0000`),
  `kernel-riscv64.ld` (virt `0x8020_0000`). `kernel/build.rs` selects by target and embeds an empty
...
audit exhaustively* has an arch boundary *clean enough that a second and third ISA are bounded drops-in* -
proven by the compiler and by three QEMU consoles, not by argument. The intense commandment audits were
