...
| [46](46-resource-invoke-has-no-reply-matched-form.md) | `resource_invoke` (syscall 31) is a SEND plus a plain `recv` - the shape 8.2's `CallDeadline` amendment condemns. That amendment fixed the NAMED-PEER path and left the RESOURCE path on the primitive it had just condemned. The shell is exempt only because it serves nobody | Recorded (26.7); `gs::cap` NOT started | a `ResourceInvokeDeadline` - but a new syscall is the operator's gate, and needs an 8.2 amendment |
