...
| [58](58-a-camelcase-name-in-a-comment-is-invisible-to-the-gate.md) | `comment_symbol_check` cannot see a CamelCase name in a comment, so a dead TYPE or enum variant is never caught - `UsbExclusive` was dead, cited in the present tense, and invisible. Measured cost of closing it: 15 names over 23 sites, about half ARM/DWC2 register FIELDS that belong in the baseline forever, about half apparently real rot | Open; measured, not started | enable the CamelCase pattern, then triage all 23 sites BEFORE seeding - putting findings in the baseline to keep the gate green is the one thing a ratchet must not be used for |
