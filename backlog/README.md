...
| [45](45-the-published-book-has-no-stdlib-section.md) | The published book (`website/`) has no page for `stdlib/rust`, so the one thing a newcomer most wants - how do I write a program for this OS - is what the site does not show. Blocks the Stranger Test (22.7), whose premise is the PUBLIC docs | Recorded (26.7) | write the page once the surface stops moving; follow the `{{#include}}` discipline, do not copy prose |
...
| [47](47-selfcheck-is-at-the-baked-script-ceiling.md) | `scripts/selfcheck.gsh` is 65,139 bytes against the 65,536 baked-script ceiling - 397 bytes of headroom. `prescan_fns` indexes `fn` definitions with u16 offsets, so a larger script would wrap silently; caught at COMPILE time, so the failure is a loud build break | Recorded (26.7); nothing broken today | SPLIT it along its `# =====` sections using the `LIBRARY` table the shell already has - the pass/fail tally lives in the shell, not the script. Widening to u32 is the fallback; moving checks to QEMU-only suites is rejected (selfcheck runs on HARDWARE) |
