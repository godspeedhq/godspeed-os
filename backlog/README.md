...
| [41](41-a-throwaway-shell-for-the-commands-that-cannot-detach.md) | a throwaway shell would let `selfcheck` and `run` detach too - feasible in about two days, and it would hand a background job the SHELL's whole authority, which is what the job design rejected option B for. Blocked on invocation-scoped `spawn` delegation, not on effort | Open by decision (26.2) | a real need, and `spawn` that can narrow caps |
