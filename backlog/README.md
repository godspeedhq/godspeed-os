...
| [40](40-selfcheck-is-out-of-room.md) | `selfcheck.gsh` 376 bytes from a hard u16 ceiling, and NEGATIVE on Windows because `*.gsh text` converted to CRLF - **CLOSED 2026-09-24** by the split (`backlog/47`); nothing was shaved, which is what this entry existed to prevent. The `eol=lf` fix stands | Closed | - |
...
| [45](45-the-published-book-has-no-stdlib-section.md) | The published book had no page for `stdlib/rust` - **CLOSED 2026-09-24**: `website/src/stdlib.md` ships, and `pages.yml` did not list `stdlib/` at all, so a corrected doc comment would never have reached the published API reference | Closed | - |
| [46](46-resource-invoke-has-no-reply-matched-form.md) | Resource-cap invocation has no bounded, correlated reply: `resource_invoke` is a SEND plus a plain `recv`, so a serving caller drops unrelated traffic (hole 1) and the owner's death never wakes it (hole 2 - `examples/holder` does a bare `ctx.recv()`). GENERIC, not file-specific: 3 issuers, and it predates the stdlib | HOLE 1 CLOSED in userspace for EVERY issuer (fs file caps and net-stack socket/listener/connection caps all tagged; `gs::cap` built on it). HOLE 2 - no `ReplyDead` for resource invocation - remains, and is all that is still live here | investigate `resource_invoke` independently of `gs::cap`; syscall count is not responsibility count, so the question is whether a MISCIS semantic is incomplete |
| [47](47-selfcheck-is-at-the-baked-script-ceiling.md) | `selfcheck` was 65,139 bytes against a 65,536 ceiling - **CLOSED 2026-09-24**: nine parts under `scripts/selfcheck/`, largest 13,367. The entry was wrong twice: the driver cannot be a script (nesting is refused) and the tally was a local of one `run_lines` call, not shell state | Closed | - |
| [48](48-chaos-max-carnage-panicked-the-kill-path-bound.md) | `chaos max-carnage` panics the kernel on the kill path's 0.75 s progress bound - **REPRODUCED under host load** (1 in 170 rounds loaded, 0 in 400 idle). A core stopped inside a few-instruction window looks identical to a wedged one. The panic is the right of two bad choices AT that point; the defect is that a kill that cannot complete kills the MACHINE rather than failing the kill | Reproduced; the fix is a design decision | fail the kill instead of panicking; re-send the WAKE_RECEIVER IPI inside the spin; print the stuck core's IRQ count |
| [49](49-five-wedge-detectors-halt-the-machine-on-lateness.md) | FIVE kernel panics fire on LATENESS, not on a fault they established: the kill path, the liveness watchdog, TLB shootdown, the spinlock and the frame allocator - three of them bounded by a SPIN COUNT. `note_irq` already stamps the per-core interrupt count that would tell late from stuck, and none of them reads it | Recorded (26.7); 1 of 5 proven | fail the OPERATION not the machine; check the stuck core's IRQ count; bound by time, not spins |
| [50](50-nic-driver-read-100-percent-on-the-t630.md) | `nic-driver` read 100% CPU on the T630 after a run of network tests. NOT this branch (nic-driver is byte-identical to main; the net-stack diff touches no yield/poll/loop) and NOT the idle state in QEMU, where it sits in `BlockRecv`. Three live candidates: transient device waits, RTL-specific paths, or the sampled CPU%% resonating with the 10 ms tick | Recorded (26.7); one sighting, one machine | ask three questions at a prompt before bisecting anything - the entry lists them |
| [51](51-a-display-only-status-region-in-the-console.md) | DESIGNED, not built: a display-only status region the `console` owns, so ANY long-running command can show a bar while output scrolls above. Three attempts to give `selfcheck` a progress display failed because a mode of one command is the wrong unit, and because `ConsoleWrite` writes serial AND the TV - so a painted frame costs the transcript. Over the console's IPC path it renders to the framebuffer only | Designed; deferred until `feat/stdlib` closes | settle the region's OWNERSHIP first (a 7.10 resource cap, revoked on death) and bound the update rate in the design |
| [52](52-the-shell-suite-flakes-on-sock-and-serve.md) | `osdev test shell` gave 206/0, 205/1 and 199/7 on a tree whose only difference was COMMENT TEXT (0 non-comment lines changed). Failures cluster on `sock` and `serve` - the only two tests that depend on a peer outside QEMU - and the three `scrollback` cases that follow `serve` are a CASCADE, not a third fault, which makes one flaky test read as seven | Open; not root-caused, one machine, one session | instrument both tests to report the ELAPSED time on failure, so "never arrived" and "arrived after we stopped listening" stop looking identical |
| [53](53-the-build-workflow-is-four-months-stale.md) | `build.yml` is paused (workflow_dispatch only, last green 2026-05-24) and fails to BUILD rather than failing to RUN, so dispatching it reports a red X on a healthy tree - and a red run paints the branch red, because GitHub aggregates checks by SHA. One layer fixed (supervisor built after the services it embeds); a deny-by-default clippy lint and at least one more layer remain | Open; not chased further - it is paused for lack of CI minutes and reviving it spent 20 of them | cheapest fix is to make it exit IMMEDIATELY with "paused and unmaintained, see backlog/53", which stops it being a trap without deciding revive-vs-delete |
| [54](54-stack-fit-is-blind-on-a-frameless-binary.md) | `stack_fit_check` reports itself BLIND on `examples/00-hello`, on the reasoning that "every function has a zero frame, which no real service has". Verified: `hello` genuinely has zero `sub rsp` in the whole binary - it is 43 lines that inline away. So the heuristic is right about services and wrong about a minimal example, and the warning is a false positive | Open; PRE-EXISTING, confirmed against the pre-migration binary | either exempt binaries below a size floor, or accept a genuinely-frameless binary when its symbol count is tiny |
| [55](55-the-released-crates-carry-the-previous-version.md) | v0.20.0 shipped every crate labelled `0.19.0`: `workspace.package.version` was last bumped for v0.19.0 and `git show v0.20.0:Cargo.toml` still reads `0.19.0`. rustdoc prints the crate version on every page, and the README sends a newcomer to those pages as the canonical API - so the first fact they read is a superseded version. Ten doc gates pass and are right to: no document restates the version, so nothing compares the tag with the manifest | **CLOSED 2026-09-26** - manifest is 0.21.0 and `release.yml` refuses a mismatched tag | bump it as part of cutting a release AND assert tag == `Cargo.toml` version in `release.yml`, so it fails before anything is published |
| [56](56-fs-shouts-ten-times-about-an-expected-transient.md) | `fs` prints `flash REFUSED - block-driver reports 0 capacity` TEN times in ~550 ms at every boot on the Wyse and the Pi 2, then mounts fine. The behaviour is right - the driver has not finished IDENTIFY, so it truthfully answers 0 and `fs` refuses to guess (invariant 12) - but a warning repeated ten times for an expected transient is one that gets scrolled past, and it is the only line that says "no disk" when that is real | Open; cosmetic, pre-existing, seen on 2 boards | latch it to one line per boot (as the SDK already does for metric-name truncation); the honest fix is for `block-driver` to announce readiness so `fs` waits on truth rather than retrying on a timer (Commandment VIII) |
| [57](57-the-flush-verdict-cannot-see-past-block-driver.md) | `fs` reports "this drive ANSWERED and refused the cache flush" when the drive was never reached. SIX sightings across the Pi 4 (aarch64) and VisionFive (riscv64), and all six have an unreachable `xhci` immediately before them - `block-driver` is alive and answers promptly, so `fs` reads its "could not ask" as the DEVICE's verdict. This is the message CLAUDE.md 6.1 nominates as the evidence for amending the durability claim, so a false positive here launders rather than merely misleads | Open; reproduced on 2 boards / 2 ISAs, zero outside chaos | carry a fourth value `Unreachable` in the flush reply - `block-driver` already prints the distinction two lines earlier - then re-run; until then a `Refused` seen under chaos must not be cited |
...
| [58](58-a-camelcase-name-in-a-comment-is-invisible-to-the-gate.md) | `comment_symbol_check` cannot see a CamelCase name in a comment, so a dead TYPE or enum variant is never caught - `UsbExclusive` was dead, cited in the present tense, and invisible. Measured cost of closing it: 15 names over 23 sites, about half ARM/DWC2 register FIELDS that belong in the baseline forever, about half apparently real rot | CLOSED same day - enabled + 21 sites triaged | enable the CamelCase pattern, then triage all 23 sites BEFORE seeding - putting findings in the baseline to keep the gate green is the one thing a ratchet must not be used for |
| [59](59-set-clock-is-a-capability-nothing-checks-and-a-log-line-that-blames-the-kernel.md) | `SET_CLOCK` (ResourceId 13) is registered and minted from two privilege bits and **checked by nobody** - no syscall sets a clock and nothing calls any arch's `set_wall_clock`; clock slice 3 gave the wall clock to the `time` service and left the kernel seam behind. Separately, net-stack logs "clock set REFUSED by the kernel (no SET_CLOCK cap)" for a refusal the KERNEL never made - the request went to `time` by IPC - and prints it for an unreachable peer as well as a real refusal (the `backlog/57` shape) | Open; both ends read, neither fixed | remove the dead capability + privbits + 7 `set_wall_clock` stubs (spawn-path change, wants hardware); and carry `time`'s own verdict into the log line |
