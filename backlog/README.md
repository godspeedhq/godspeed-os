...
| [48](48-chaos-max-carnage-panicked-the-kill-path-bound.md) | `chaos max-carnage` panicked the kernel on the kill-path bound once in four runs, in QEMU/TCG on a loaded host, with the kernel byte-identical to main. The bound is a COUNT of another core's ticks, so a DESCHEDULED vCPU and a WEDGED one look the same from it - the panic text does not say which core 2 was | Recorded (26.7); one sighting | reproduce under load, then print whether the stuck core's timer was ticking at all |
