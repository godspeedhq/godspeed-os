...
//!
//! This module was called `cap` until the standard library grew general capability
//! operations. It never held any: it holds [`File`] and nothing else, so the name promised
//! a capability API and delivered a file one - which is why `cap-grant` and `holder` reached
//! past it into the SDK. The rights constants moved to [`crate::cap`], where `cap::READ`
//! reads as what it is: a capability right (CLAUDE.md 7.4).
//!
//! # What this is
//!
