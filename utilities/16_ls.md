...
```
ls 0.4.0 - list a directory (records when piped)

usage:
  ls                       list the current directory
  ls <path>                list the directory at <path>
  ls long                  one entry per line with type, size and MODIFIED time
...
The words combine, in any order, and mix with a path: `ls long human /projects` and
`ls /projects human long` are the same command. **Words, not flags** (`0_conventions.md` rule 4) -
...
`ls` is read far more often than it is studied. The common use is "what is in here", and a wall of
...
a file written before GSFS0009, or one written while the machine had no idea what time it was.
