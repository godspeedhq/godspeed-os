...
def main():
    ...
    # ADDITIVE, not a replacement. `--feature X` used to SUBSTITUTE for the boot-path feature,
    # producing a kernel with no `arm-supervisor` - which builds fine and boots into nothing. The
    # boot path is not optional, so anything passed rides WITH it.
