...
# and runs `run /smoke.gs` — proving the script-disk → run-from-file path (including a piped
# assert, which a file can carry but on-device `write` cannot author). The extensive coverage
# lives in the embedded suite (`selfcheck` / scripts/t630_selfcheck.gs). Passes iff "failed 0".
assert ok echo hi
