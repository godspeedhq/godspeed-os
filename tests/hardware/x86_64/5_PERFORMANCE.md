...
## B2 — not measured on Goldmont+

Same root cause as `12_PERFORMANCE_BRUTAL.md`: the cross-core WAKE_RECEIVER IPI from
core 1 → core 0 is not reliably delivered under concurrent probe IPI traffic on this
hardware (Goldmont+ BSP APIC quirk). B2 probes spawn correctly on cores 0/1 but the
round-trip never completes.

Cross-core IPC is correct — ping/pong runs continuously on this hardware. See
`12_PERFORMANCE_BRUTAL.md §BP2` for the full root-cause writeup.
