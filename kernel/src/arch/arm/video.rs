...
/// Map the framebuffer Normal non-cacheable (`mmu::section_fb` explains why it is not cacheable).
/// Called AFTER `mmu::enable`, once translation is on - the counterpart to the MMU-off `request`. The
/// boot/panic floor (`crate::bootcon`) draws into it after this, as does the `console` service.
