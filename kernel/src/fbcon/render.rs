...
//! supplies the slice and publishes the writes afterwards (`super::commit`).
//!
//! **No `unsafe` lives here, by design.** §18.1 permits `unsafe` only in `arch/`, `memory/`,
//! `capability/` and `smp/`, and this is none of those - so the single `unsafe` needed to turn a mapped
//! framebuffer address into a slice sits in each arch backend, where the mapping's validity and
//! lifetime are actually known. Everything below is bounds-checked slice work: a wrong offset paints
//! nothing rather than corrupting memory, and the checks are hoisted to one per row so the hot paths
//! keep writing contiguous runs.
//!
//! Two glyph sources:
...
/// on a denser panel.
pub(super) const FONT_WEIGHT: FontWeight = FontWeight::Regular;
pub(super) const RASTER_HEIGHT: RasterHeight = RasterHeight::Size20;
pub(super) const CELL_W: usize = get_raster_width(FONT_WEIGHT, RASTER_HEIGHT);
pub(super) const CELL_H: usize = RASTER_HEIGHT.val();
...
pub(super) fn cell_for_codepoint(cp: u32) -> u8 {
    if cp < 0x80 {
        return cp as u8;
    }
    match cp {
...
/// seamlessly.
pub(super) fn box_arms(ch: u8) -> (bool, bool, bool, bool) {
    match ch {
...
///
/// The dirty rectangle is what `super::commit` hands the arch at the end of a locked batch. An arch
/// whose framebuffer is cacheable (ARM) cleans exactly this region to the Point of Coherency so the GPU
/// sees it; an arch with a write-combining mapping (x86) ignores the region and just fences. Tracking a
/// bounding box rather than each rectangle keeps this to four comparisons per draw, and over-covering is
/// always *correct* - cleaning an already-clean line costs a little time and changes nothing.
#[inline]
pub(super) fn dirty(s: &mut Fb, x: usize, y: usize, w: usize, h: usize) {
    if w == 0 || h == 0 {
        return;
    }
...
/// `super::init` precomputes once (foreground scaled per channel, composed in the device's layout), so
...
#[inline]
pub(super) fn blend(s: &Fb, intensity: u8) -> u32 {
...
/// Write a single pixel at (x, y) in the device's pixel layout. Does NOT mark the pixel dirty - callers
/// draw whole rectangles and mark those, so publication stays one bounding box per batch.
#[inline]
pub(super) fn put_pixel(s: &mut Fb, x: usize, y: usize, color: u32) {
...
pub(super) fn fill_rect(s: &mut Fb, x: usize, y: usize, w: usize, h: usize, color: u32) {
    if x >= s.width || y >= s.height {
        return;
    }
...
pub(super) fn clear_all(s: &mut Fb) {
    if let Some(mem) = s.mem.as_deref_mut() {
        mem.fill(0);
    }
    for r in 0..super::MAX_ROWS {
        for c in 0..super::MAX_COLS {
            s.grid[r][c] = b' ';
        }
    }
...
pub(super) fn draw_glyph(s: &mut Fb, ch: u8, col: usize, row: usize) {
    let sc = cell_scale(s);
...
pub(super) fn clear_cell(s: &mut Fb, x0: usize, y0: usize) {
    let sc = cell_scale(s);
    let color = paper(s);
...
pub(super) fn draw_box_glyph(s: &mut Fb, ch: u8, x0: usize, y0: usize) {
    clear_cell(s, x0, y0);
    let sc = cell_scale(s);
    ...
    // the cross is solid. Horizontal arms span the stroke rows [hy, hy+th). `clear_cell` above already
    // marked the whole cell dirty and every stroke lies inside it, so `stroke` need not mark again.
    if up {
