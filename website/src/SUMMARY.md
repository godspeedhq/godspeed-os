# Summary

[Introduction](introduction.md)

# The Constitution

- [The Constitution (CLAUDE.md)](constitution.md)
- [The Ten Commandments](commandments.md)
- [Glossary](glossary.md)
- [The Almanac](almanac.md)

# The Standard Library

- [Writing a program for GodspeedOS](stdlib.md)
  - [Design report: what building it changed](design/stdlib.md)
  - [The brief it was built from](design/stdlib-brief.md)

# The System

- [The services, and how they reach each other](services.md)
- [The utilities](utilities.md)
  - [Conventions](utilities/conventions.md)
  - [`observe`](utilities/observe.md)
  - [`date`](utilities/date.md)
  - [`mem`](utilities/mem.md)
  - [`cores`](utilities/cores.md)
  - [`about`](utilities/about.md)
  - [`caps`](utilities/caps.md)
  - [`status`](utilities/status.md)
  - [`echo`](utilities/echo.md)
  - [`clear`](utilities/clear.md)
  - [`spawn`](utilities/spawn.md)
  - [`kill`](utilities/kill.md)
  - [`restart`](utilities/restart.md)
  - [`reboot`](utilities/reboot.md)
  - [`poweroff`](utilities/poweroff.md)
  - [`background`](utilities/background.md)
  - [`drives`](utilities/drives.md)
  - [`dir`](utilities/dir.md)
  - [`cd`](utilities/cd.md)
  - [`read`](utilities/read.md)
  - [`write`](utilities/write.md)
  - [`mkdir`](utilities/mkdir.md)
  - [`copy`](utilities/copy.md)
  - [`move`](utilities/move.md)
  - [`rename`](utilities/rename.md)
  - [`delete`](utilities/delete.md)
  - [`find`](utilities/find.md)
  - [`tree`](utilities/tree.md)
  - [`match`](utilities/match.md)
  - [`count`](utilities/count.md)
  - [`sort`](utilities/sort.md)
  - [`first` / `last`](utilities/first-last.md)
  - [`records`](utilities/records.md)
  - [`result`](utilities/result.md)
  - [`run`](utilities/run.md)
  - [`assert`](utilities/assert.md)
  - [`fcap`](utilities/fcap.md)
  - [`seal`](utilities/seal.md)
  - [`edit`](utilities/edit.md)
  - [`uptime`](utilities/uptime.md)
  - [`chaos`](utilities/chaos.md)
  - [`fmt`](utilities/fmt.md)
  - [`net`](utilities/net.md)
  - [`sock`](utilities/sock.md)
  - [`tcp`](utilities/tcp.md)
  - [`serve`](utilities/serve.md)
  - [`ping`](utilities/ping.md)
  - [`version`](utilities/version.md)
  - [`wait`](utilities/wait.md)
  - [`whatis`](utilities/whatis.md)
  - [`trace`](utilities/trace.md)
  - [`events`](utilities/events.md)
  - [`churn`](utilities/churn.md)
  - [`docs`](utilities/docs.md)
  - [`paginate`](utilities/paginate.md)
  - [`scrollback`](utilities/scrollback.md)

# The gsh Language

- [The gsh scripting language](design/scripting.md)
- [The gsh formatter (fmt)](design/fmt.md)

# Design Notes

- [Persistence and the filesystem](design/persistence.md)
- [AHCI block driver](design/ahci.md)
- [IOMMU and DMA confinement](design/iommu.md)
- [Power and idle efficiency](design/power.md)
- [Naming out of the kernel](design/naming.md)
- [Typed pipes](design/pipes.md)
- [Records](design/records.md)
- [The console service](design/console-service.md)
- [The drives model](design/drives.md)
- [Service-control capability](design/service-control.md)
- [Introspection capability](design/introspection.md)
- [The unsafe audit](design/unsafe-audit.md)
- [Licensing](design/licensing.md)

# Forward-Looking

- [Networking](design/networking.md)
- [Cluster mode](design/cluster.md)

# The System, Running

- [Gallery](gallery.md)

# Contributing

- [How to contribute](contributing.md)
