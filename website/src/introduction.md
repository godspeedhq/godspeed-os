...
This site is a rendered view of the project's own documents. Eighty of its eighty-five pages -
...
to the docs themselves). The five exceptions are written for the site because they have no file to be
a view of: this page, the [Gallery](gallery.md), the [Services](services.md) tour, the
[Utilities](utilities.md) index and [Writing a program for GodspeedOS](stdlib.md). Those five are
checked against the repository on every build instead - that a utility has a row, that a service's
peers match its contract - because prose someone wrote is prose that can go stale.
