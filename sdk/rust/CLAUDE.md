...
| `trace.rs`            | The IPC trace wire format + emission arming (`utilities/46_trace.md`). A service emits only if its contract granted `ipc_send = ["logger"]`, so tracing is AUTHORITY, not a switch; the ring itself lives in the `logger` service and the kernel records nothing |
