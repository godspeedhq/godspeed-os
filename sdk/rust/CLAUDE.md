...
| `trace.rs`            | The IPC trace wire format + emission arming (`utilities/46_trace.md`). A service emits only if its contract granted `ipc_send = ["events"]`, so tracing is AUTHORITY, not a switch; the ring itself lives in the `events` service and the kernel records nothing |
