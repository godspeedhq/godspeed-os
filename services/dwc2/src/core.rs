...
pub fn reset_and_host_mode(ctx: &ServiceContext, mmio: &Mmio) -> bool {
    ...
    // `fs` sat waiting behind it looking for all the world like a cross-core IPC failure. A diagnostic
    // that changes the behaviour under test is worse than no diagnostic, because it produces confident
    // wrong conclusions - I drew one from it before spotting this.)
