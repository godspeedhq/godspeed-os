...
| `osdev test fs-corrupt`     | GSFS0005 integrity: corrupt a superblock byte (→ loud "no filesystem", no mount), a root directory block (→ loud dir-CRC mismatch, no garbage records), and a file **data block** (→ loud data-CRC mismatch, read refused); asserts no panic (§3.12). 11 checks |
