...
| `osdev test edit`           | Full-screen `edit` text editor over the serial console: open a new file, type/backspace/newline, save (^S) + quit (^Q), `read`-back; edit-existing (insert at start); quit-with-discard at the unsaved-changes prompt; no-arg usage. 9 checks |
...
`fs` write path, kept in lockstep
