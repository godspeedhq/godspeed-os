...
pub fn cmd_build_bare_metal() {
    clean_supervisor();
    let non_supervisor = ["logger", "console", "control", "time", "hw-enumerator", "mem-pressure", "chaos", "ping", "pong", "greet", "upper", "roster", "probe", "observe", "shell", "xhci", "ehci", "block-driver", "nic-driver", "net-stack", "fs", "counter", "reply-server", "asker", "resource-server", "holder"];
...
pub fn cmd_build_counter() {
    ...
    let status = std::process::Command::new("cargo")
        .args(["build", "--release", "-p", "supervisor",
               "--target", "x86_64-unknown-none",
               "--features", "supervisor/bare-metal,supervisor/counter-test"])
        .status()
        .unwrap_or_else(|e| panic!("failed to run cargo build for supervisor: {}", e));
    if !status.success() {
        eprintln!("build: supervisor (bare-metal + counter-test) FAILED");
        std::process::exit(1);
    }
...
/// Build for `osdev test reply-server`: the bare-metal set PLUS `reply-server` + its client `asker`.
/// Same as `cmd_build_bare_metal` but the supervisor also gets the `reply-test` feature, so it spawns
/// `examples/reply-server` and `examples/asker` - the request/reply (RPC) pair. asker sends a request
/// with an embedded reply cap; reply-server replies over it; `osdev test reply-server` asserts the
...
pub fn cmd_build_reply() {
    ...
    let status = std::process::Command::new("cargo")
        .args(["build", "--release", "-p", "supervisor",
               "--target", "x86_64-unknown-none",
               "--features", "supervisor/bare-metal,supervisor/reply-test"])
        .status()
        .unwrap_or_else(|e| panic!("failed to run cargo build for supervisor: {}", e));
    if !status.success() {
        eprintln!("build: supervisor (bare-metal + reply-test) FAILED");
        std::process::exit(1);
    }
...
/// Build for `osdev test resource-server`: the bare-metal set PLUS `resource-server` + its client
/// `holder`. Same as `cmd_build_bare_metal` but the supervisor also gets the `resource-test` feature,
/// so it spawns `examples/holder` then `examples/resource-server` - the delegated-resource-capability
/// pair (§7.10). resource-server mints a resource, narrows a READ-ONLY copy of the cap, and grants it
/// to holder; holder proves use / non-escalation / revoke, asserted by `osdev test resource-server`.
...
pub fn cmd_build_resource() {
    ...
    let status = std::process::Command::new("cargo")
        .args(["build", "--release", "-p", "supervisor",
               "--target", "x86_64-unknown-none",
               "--features", "supervisor/bare-metal,supervisor/resource-test"])
        .status()
        .unwrap_or_else(|e| panic!("failed to run cargo build for supervisor: {}", e));
    if !status.success() {
        eprintln!("build: supervisor (bare-metal + resource-test) FAILED");
        std::process::exit(1);
    }
...
fn build_blockdev_fs(fs_features: &str, bd_features: &str) {
    ...
    let non_supervisor = ["logger", "console", "control", "time", "hw-enumerator", "ping", "pong", "greet", "upper", "roster", "probe", "observe", "shell", "xhci", "ehci", "block-driver", "nic-driver", "net-stack", "counter", "reply-server", "asker", "resource-server", "holder"];
    for crate_name in &non_supervisor {
...
/// Does an ADOPTED service keep its name? (`osdev test adopt-storm`) - storms the SUPERVISOR so
/// adoption happens repeatedly on purpose, then asks whether the filesystem still works. See
/// `shell_test::run_adopt_storm` for the hypothesis and why it has never been tested.
fn run_adopt_storm_test() {
    println!("
=== adopt-storm: does an adopted service keep its name? ===");
    cmd_build_bare_metal();
    let kernel_elf = std::path::Path::new("target/x86_64-unknown-none/release/kernel");
    if !kernel_elf.exists() { eprintln!("kernel ELF not found"); std::process::exit(1); }
    let limine_dir = std::path::Path::new("tools/limine");
    ...
    disk_image::install_bootloader(limine_dir, &image_path);
    let _ = std::fs::create_dir_all("build/tests");
    let persist = "build/tests/persist_adopt_storm.img";
    std::fs::write(persist, vec![0u8; 16 * 1024 * 1024]).expect("failed to create raw disk");
    crate::shell_test::run_adopt_storm(&image_path, persist, 4);
}
...
/// AHCI disk + the `counter` service (counter-test build). The harness flashes the disk so `fs`
...
fn run_counter_test() {
    ...
    disk_image::install_bootloader(limine_dir, &image_path);
    let _ = std::fs::create_dir_all("build/tests");
    let persist = "build/tests/persist_counter.img";
    std::fs::write(persist, vec![0u8; 16 * 1024 * 1024]).expect("failed to create raw disk");
    crate::shell_test::run_counter(&image_path, persist, 4);
}

/// `examples/reply-server` exercised by its client `examples/asker` (the request/reply RPC pattern,
/// §8/§8.9). Boots the bare-metal set + reply-server + asker (reply-test build). No disk and no
/// control channel needed - the pair runs autonomously: asker sends reply-server a request carrying an
/// embedded reply cap, reply-server replies over it, asker checks the echo. The test asserts asker
/// logged `asker: reply = <N> (echo OK)` and reply-server logged `replied to a request`, no panic.
fn run_reply_server_test() {
    println!("\n=== reply-server: request/reply round-trip, driven by examples/asker (§8/§8.9) ===");
    cmd_build_reply();
    let kernel_elf = std::path::Path::new("target/x86_64-unknown-none/release/kernel");
    if !kernel_elf.exists() { eprintln!("kernel ELF not found"); std::process::exit(1); }
    let limine_dir = std::path::Path::new("tools/limine");
...
/// the reply-test build, where `asker` sends `reply-server` a request it never answers and blocks in
...
/// capabilities (§7.10, P2 file-as-capability). Boots the bare-metal set + resource-server + holder
/// (resource-test build). No disk and no control channel needed - the pair runs autonomously:
/// resource-server mints a resource it owns, narrows a READ-ONLY copy of the cap, and grants it to
/// holder; holder USEs it (read OK), over-reaches (write denied - non-escalation), and after the
/// owner revokes uses it again (CapRevoked). The test asserts holder logged all three receipts, no
/// panic - the three §7.3 properties that make a delegated resource cap a GENUINE capability.
fn run_resource_server_test() {
    println!("\n=== resource-server: mint a delegated resource cap, proven by examples/holder (§7.10) ===");
    cmd_build_resource();
    let kernel_elf = std::path::Path::new("target/x86_64-unknown-none/release/kernel");
    if !kernel_elf.exists() { eprintln!("kernel ELF not found"); std::process::exit(1); }
    let limine_dir = std::path::Path::new("tools/limine");
    ...
    disk_image::install_bootloader(limine_dir, &image_path);
    let _ = std::fs::create_dir_all("build/tests");
    crate::shell_test::run_resource_server(&image_path, 4);
}
