...
| `arch/riscv64/display.rs` | 5 -> 6 (+1) | 2026-09-08: display stage six - the boot console adopts the framebuffer. ONE more `from_raw_parts_mut`, over the SAME run of frames stage four reserved and handed to the display controller: permanently removed from the allocator (`alloc_dma_arena`), inside the identity map, with no third owner - the controller reads it and the console writes it. It is a second slice rather than a stored one because the first was moved into the controller's programming and a `&'static mut` cannot be copied; both describe the same bytes and both are the one unsafe step that keeps every glyph a bounds-checked slice write. Nothing about the font, the palette or the layout is decided in `arch/`: the port owes `bootcon` an `FbParams` and an `fb_commit` and gets the other ports' text for free, which is the arch seam's whole claim and the place a display is easiest to break by writing a small renderer instead. |
| `arch/riscv64/display.rs` | 4 -> 5 (+1) | 2026-09-08: display stage four - the mode set. ONE more line, and it is not an MMIO access: `core::slice::from_raw_parts_mut` over the framebuffer, turning 2025 physically-contiguous pages just reserved by the frame allocator into a `&'static mut [u8]`. Sound because the run is freshly allocated (so owned by nobody else), RESERVED rather than merely allocated (`alloc_dma_arena`, so it can never be returned to the pool and handed out again as a page table under a device that scans it forever), and inside the kernel's identity map. It is the one unsafe step on purpose: every pixel written afterwards is a bounds-checked slice write, which is the same shape that keeps the boot console free of `unsafe` on every other port. The controller writes themselves add no lines - they go through the two accessors stage two already declared. Register facts from StarFive's `vs_dc_hw.c` and `vs_dc.c`, read as an executable datasheet (26.14); the address is bounds-checked against 4 GiB first, because the controller's address register is 32 bits wide and a truncated one would have the display scanning out somebody else's memory. |
| `arch/riscv64/display.rs` | 2 -> 4 (+2) | 2026-09-08: display stage two - the clocks and resets. Two more MMIO accessors, this time for the two clock/reset generators rather than the PMU, and their soundness is ORDERING rather than address: `syscrg` is in the always-on part of the SoC, but `voutcrg` is inside the VOUT domain, so it is touched only after the power domain reports on and the system clocks feeding it are enabled. That order is the whole content of the file. Every enable is READ BACK, because a write into an unpowered block is silence rather than an error, and every reset deassert is BOUNDED - the reference driver notes that deasserting a reset whose clock is still gated never completes, so the bound is what turns an ordering mistake into a reported failure instead of a dead machine. Register facts from `clk-starfive-jh71x0.h`, `starfive,jh7110-crg.h` and `reset-starfive-jh7110.c`. |
| `arch/riscv64/display.rs` | 0 -> 2 (new file) | 2026-09-08: display bring-up, first stage - the VOUT power domain. TWO lines, a volatile read and a volatile write of the JH7110 PMU at 0x1703_0000, at the address the device tree gave and inside the identity map. Sound at any time for a reason that is the whole point of starting here: the PMU is in the ALWAYS-ON block, so its registers answer whatever else on the SoC is powered down. Nothing in this file touches the display controller, the video-out clock controller or the HDMI transmitter, because all three sit INSIDE the domain being switched on and a read there while it is unpowered is a bus transaction with nothing to answer it rather than a zero. The only registers written are the power request and its three-write encourage sequence, which is the PMU's documented interface for exactly this (Linux `drivers/pmdomain/starfive/jh71xx-pmu.c`, read as a datasheet per 26.14). |
| `arch/riscv64/mod.rs` | 57 -> 59 (+2) | 2026-09-08: the early-boot log replay, so the television shows the whole boot rather than joining it at the first timer tick. TWO lines, both over one fixed 16 KiB array: the copy into it while there is no screen, and the slice over it when there is. The copy is exclusive because it holds the same flag the painting path holds - a byte is either kept for later or drawn now, never both and never torn between cores - and its length is clamped to the space remaining, so a boot that talks more than expected loses the tail of the replay rather than reaching past the array. The read happens before the screen is armed, so nothing else is in the array at the time. The kernel's own log ring is NOT an alternative: the arch's `print_str` writes the UART directly and never enters it, and draining it here would take it from the `events` service, which is the one thing it exists for. |
| `arch/riscv64/context_switch.rs` | 34 -> 35 (+1) | 2026-09-10: the `next.sp` read, for the kernel stack guard beside the root guard. Worse to get wrong than the address space: a trap taken from S-mode runs on whatever `sp` it finds, so a garbage kernel stack does not fault once but forever - the fault handler's own prologue faults on the same bad pointer before reaching an instruction that could report it. The read is of the same `next` the caller already guarantees valid, one line from the `cr3` read above it. |
| `arch/riscv64/context_switch.rs` | 30 -> 34 (+4) | 2026-09-10: `switch_context` is SPLIT, taking the shape `arch/aarch64` already has - ordinary Rust installs the address space, then a naked `riscv64_switch_registers` does the registers alone. The four lines are the `next.cr3` read, the `phys_in_ram` guard's two reads, and the `csrw satp` + `sfence.vma` that used to live in the asm. The point of the split is that naked assembly cannot call anything, so it could neither CHECK the root nor SAY anything when it was impossible - and installing a reclaimed root leaves the hart executing where the kernel's text is unmapped, unable even to fetch the handler that would report it. In Rust the check is a call and the failure is a panic: bounded and loud, where the old shape was a hart that simply vanished. |
| `arch/riscv64/mod.rs` | 66 -> 67 (+1) | 2026-09-10: `note_stage` reads its own `ra`, so the wedge dump prints WHERE a stage was stamped from and not only which stage. Three times in one hunt a stage was read as a location and proved ambiguous - 14 meant both wedged-in-the-drain and halted-by-a-neighbour, 15 meant both before and after the progress stamp, and 17 sat on `syscall_slot`, which has eight call sites. A return address cannot be ambiguous. One `mv {}, ra`: no operands, no memory effect, meaningful only because the function is `#[inline(never)]`, which is stated at the block. |
| `memory/allocator.rs` | 46 -> 47 (+1) | 2026-09-10: `ram_limit_phys` - the top of RAM as a NUMBER, beside the `phys_in_ram` that already answers the same question as a call. It exists because one caller cannot make a call: `arch/riscv64`'s `switch_context` is naked assembly, and the page-table root it installs into `satp` has to be checked BEFORE the write or the hart executes into an address space that may have been reclaimed - a hardware fault loop with no output, since `stvec` points into the same dead space. One read of `max_ram_frame`, which is set once at init and never mutated, exactly as its neighbour does. Deriving it here rather than in the arch keeps the two from disagreeing. |
| `arch/riscv64/mod.rs` | 65 -> 66 (+1) | 2026-09-10: `panic_halt_check` gains a `wfi` halt loop. x86 leaves this seam member an empty stub because it stops its siblings with an NMI broadcast; RISC-V has no NMI, so leaving it empty here left `halt_all_cores` halting only the hart that called it - a panicking machine kept running services on three harts while one printed its post-mortem. Every hart takes a timer tick within a quantum, so polling a flag here reaches all of them without an IPI a wedged hart could not answer. The block is one `wfi`: a hint with no operands and no memory effect, reached only when a panic is already in flight and the machine is being taken down. |
| `arch/riscv64/mod.rs` | 58 -> 57 (-1) | 2026-09-08: `evict_by_filling` is DELETED. It existed to answer a question the serial console could not - whether the vendor's cache flush did anything - by putting a second method on the kernel's own drawing while the vendor's stayed on the service's, so the screen showed both at once. It answered: the vendor's flush was pointed at the wrong one of the cache controller's two windows. With that corrected the vendor's method is both correct and thirteen times cheaper (154 microseconds against 2089), and it has no ordering hazard - the eviction fill races dirty L1 lines draining into L2 while it is filling L2, which left a band. A diagnostic that has delivered its answer is deleted rather than kept in case. |
| `arch/riscv64/mod.rs` | 57 -> 58 (+1) | 2026-09-08: `evict_by_filling` - a second way to push the framebuffer out of the last-level cache, using nothing but ordinary memory. ONE block: a read loop over a run of RAM this module reserved at boot (`alloc_dma_arena`, so nothing else can ever be given it), inside the identity map, READ only, values discarded. `read_volatile` is what stops the compiler deleting a loop whose results are unused - which would delete the whole point of it. It exists because whether the vendor's flush works depends on whether the cache controller's zero-device window is cached, which is a fixed platform memory attribute this port cannot read; reading twice the cache's size of ordinary RAM has no such question in it. |
| `arch/riscv64/mod.rs` | 54 -> 57 (+3) | 2026-09-08: the last-level cache flush, which is what `fb_commit` actually owed. Three lines: the two config reads in `ccache_init`, and the one block in `ccache_flush_all` holding the way-mask writes, the zero-device writes and the fences. Both windows come from the cache controller's own device-tree node and are inside the identity map. The soundness argument that matters is not the addresses but the RESTORE: the flush confines every bus master to one cache way at a time, and a path that left it confined would produce a machine that still works and is sixteen times smaller, so the mask is put back on every exit including a loop entered zero times. THIS PART CANNOT FLUSH BY ADDRESS - its controller defines a flush-by-physical-address register and the vendor driver never uses it because it does not work on this SoC - so the only granularity available is the whole cache, and `fb_commit`'s rectangle is information the hardware cannot use. The ISA offers nothing either: neither `Zicbom` nor `Svpbmt` is implemented, so there is no cache-block operation and no non-cacheable page attribute. Recorded rather than hidden (26.7). |
| `arch/riscv64/mod.rs` | 53 -> 54 (+1) | 2026-09-08: `fb_commit` gains a body - one `fence rw, rw`, which has no operands and no effect beyond ordering. The framebuffer is ordinary cacheable RAM here because there is nothing else available: this part implements neither `Svpbmt` (no page-based memory type to mark a page non-cacheable) nor `Zicbom` (no cache-block op to clean one), and its device tree lists neither. So what the seam owes on this port is ORDERING, which is the correct and complete answer where DMA is coherent - and this board's tree says it is, marking no node `dma-noncoherent` and several explicitly `dma-coherent`. If that is ever proved wrong, this is where it shows and S-mode here has no instruction to fix it; recorded rather than assumed away (26.7). |
| `arch/riscv64/uaccess.rs` | 7 -> 8 (+1) | 2026-09-08: `read_cycle_counter` reads `cycle` where the machine permits it and `time` where it does not, so the two CSR reads are two lines instead of one. Both are side-effect-free reads. The MAGNITUDE is what the change is for: userspace budgets are cycle counts written against a gigahertz-ish counter, and answering with a 10 MHz timebase turned `block-driver`'s 400-million-cycle AHCI link wait into forty seconds per port. Nothing in that driver is wrong at that point - the unit is. |
| `arch/riscv64/mod.rs` | 50 -> 53 (+3) | 2026-09-08: PCI Express through ECAM, and the `rdcycle` probe. Configuration space is MEMORY on this ISA - there is no port I/O, so a config read is one volatile read at an address built from bus/device/function, where x86 needs a write to CF8 then a read from CFC. The window's base comes from the device tree, never a constant, and every access is bounded by the size the tree declared, so a wrong `bdf` returns `None` instead of touching memory outside the window. The third is the probe: `cycle` is readable from S-mode only if M-mode set `mcounteren.CY`, there is no bit that says so, and getting it wrong is an illegal instruction rather than a zero - so the instruction is executed DELIBERATELY with the trap handler armed for exactly one of them, the same shape as the user-mode selftest's deliberate page fault. |
| `arch/riscv64/mod.rs` | 64 -> 65 (+1) | 2026-09-09: `note_syscall` reads `tp` for the hart id, the same one-line register read as `note_irq` and `note_stage` beside it. It exists because a stage alone was not enough: the wedge dump showed core 0 at stage 1 (`trap-entry`) with neither `timer-enter` nor `fault-report` set, which rules out the SBI calls and the whole fault-reporting path and leaves the one route through the trap handler that carried no stamp - `syscall::dispatch`. Recording the NUMBER costs the same store and answers a different question: "stuck in a syscall" leaves sixty candidates, "stuck in syscall 41" names the code to read. |
| `arch/riscv64/mod.rs` | 65 -> 64 (-1) | 2026-09-09: the `tp` read added so the framebuffer publish could tell which core it was on is GONE, because restricting the publish to one core was the wrong lever. The race it addressed is real - `ccache_flush_all` reprograms every bus master's way-mask - but the fix for a race is mutual exclusion, which the try-lock inside the flush already provides. Cutting the number of CALLERS also cut the publish RATE fourfold, which on a framebuffer this SoC cannot keep coherent is visible tearing, and the operator saw it. The pacing is now a clock and the safety is a lock, which is what each is for. A reduction, so no amendment (18.5). |
| `arch/riscv64/mod.rs` | 64 -> 65 (+1) | 2026-09-09: one more `tp` read, so the framebuffer publish can tell which core it is on. `timer_tick` runs on every hart, so the publish ran on every hart - four whole-L2 flushes every two ticks for one framebuffer with one writer, and that flush reprograms the way-mask of every bus master on the SoC. Restricting it to core 0 needs the core id, and the id comes from `tp`. Mapped through `hart_core` rather than compared as a hart number, because on this board the boot hart is 1. |
| `arch/riscv64/mod.rs` | 61 -> 64 | 2026-09-09: the IDLE SAMPLE - two CSR reads (`sie`, `sip`) and two `tp` reads, taken immediately before a hart executes `wfi`. All four are side-effect-free reads. They exist because a hart that never wakes has no "after": everything not captured before the `wfi` is unavailable forever, which is why three attempts at this wedge had nothing to read. The four numbers separate the only three explanations a dead `wfi` has - never armed, not enabled to wake (STIE clear), or armed and enabled and undelivered. |
| `arch/riscv64/mod.rs` | 60 -> 61 (+1) | 2026-09-09: `note_stage` reads `tp` for the hart id, the same one-line register read as `note_irq` beside it and for the same reason - S-mode has no register that reads back which hart is executing. It stamps a phase so the liveness watchdog can say WHERE a core stopped rather than only that it did: the wedges caught so far name two different tasks on two different cores, which rules out one guilty service and leaves the kernel path they share, and the interrupt counter freezing mid-run proves the core entered a trap and never left it. One relaxed store on a path that already does several. |
| `arch/riscv64/mod.rs` | 59 -> 60 (+1) | 2026-09-09: `note_irq` reads `tp` to learn which hart it is on. One line, a register read with no side effects, on a path that already had to know: the hart id lives in `tp` because RISC-V S-mode has no register that reads back "which hart am I". It earns its place by replacing a STUB that returned `(0, 0)` - and that stub printed "0 timer interrupts" inside the first liveness-wedge panic this port ever produced, about a core whose timer nobody had ever counted. Zero is the most incriminating answer that message can carry and it was not a reading. x86 carried the same stub, and making it real is what let a repro there distinguish a timer that STOPPED from a tick that was merely SKIPPED. |
| `arch/riscv64/sbi.rs` | 12 -> 10 (-2) | 2026-09-09: `remote_fence_i` is DELETED, and the deletion is the fix. It broadcast `fence.i` to every hart so a spawn could publish the text it had just written - the right guarantee, in a place that cannot afford it. A broadcast waits for every target to acknowledge, and it was issued from inside a spawn, which runs with interrupts off: the same hazard `task/scheduler.rs` documents for TLB shootdowns and refuses to take. It survived three supervisor respawns and hung the machine on the fourth, three rounds into a chaos run, with the log stopping between "respawning" and "spawned OK" and no panic, because nothing had failed. The guarantee moved to a LOCAL `fence.i` in `context_switch::user_entry_trampoline`, on the hart about to run the task, one instruction before it does - same property, no cross-hart wait, no dependency on the RFENCE extension, and cheaper. A reduction, so no amendment is needed (18.5). |
| `arch/riscv64/sbi.rs` | 10 -> 12 (+2) | 2026-09-09: `remote_fence_i`, which publishes freshly loaded CODE to every hart. Two lines: the local `fence rw, rw` + `fence.i`, and the RFENCE `ecall` that makes the other harts do the same. Both are ordering operations with no memory effects - `fence.i` discards fetched instructions on the hart that runs it, and the firmware call runs the same instruction on the harts it names. What makes them NECESSARY rather than defensive is that RISC-V has no coherence between the data path and the instruction path and no broadcast form of the fence: the loader writes a service's text with ordinary stores, and a DIFFERENT hart then executes it. Without this, that hart runs whatever those physical lines held before, which on a recycled frame is a dead service's text - observed on the VisionFive as `roster` and `upper` trapping at a fixed `sepc` on a fixed hart while the identical spawn succeeded on another. |
| `arch/riscv64/uaccess.rs` | 8 -> 7 (-1) | 2026-09-09: `read_cycle_counter` reads `time` and only `time`, so the two CSR reads become one again. `cycle` counts CPU cycles and therefore STOPS in `wfi`, which makes a cycle-denominated deadline unable to elapse while a core is idle - the exact shape of a wait that never returns. `time` advances at a constant rate whatever the core is doing, which is what every duration above the kernel actually means. A reduction, so no amendment is needed (18.5). |
| `arch/riscv64/sbi.rs` | 6 -> 10 (+4) | 2026-09-08: SBI HSM and IPI - how a secondary hart is STARTED on this ISA and how one hart pokes another. `call3` is the three-argument form HSM's `HART_START` needs; `hart_start` and `send_ipi` are safe wrappers over it that PROBE the extension first and report a firmware refusal rather than assuming success - a hart that never started is otherwise indistinguishable from one that started and hung, and those have opposite fixes. Neither call can affect the calling hart. There is no trampoline and no INIT/SIPI timing here: the whole of x86's real-mode `ap_boot` is replaced by one firmware call, which is also why the started hart arrives with `satp` zero and must repeat `_start`'s work itself. |
| `arch/riscv64/trap.rs` | 6 -> 7 (+1) | 2026-09-08: `enable_software_interrupts` sets `sie.SSIE`, which is what admits an IPI. Separate from the timer enable because the two are wanted at different moments - the boot hart needs the timer long before any other hart exists, a secondary needs both at once - and sound for the same reason: `stvec` is installed before either, so an admitted interrupt always has somewhere to go. |
| `arch/riscv64/mod.rs` | 39 -> 50 (+11) | 2026-09-08: SMP. The AP entry (naked: `tp` for this hart's identity, `sp` for the stack reserved for it, jump), `get_lapic_id` reading `tp` and the two writes that set it, the `satp` install on a secondary, the `sip.SSIP` acknowledgement, the IPI drain calling the neutral handler, and `start_all_aps`. RISC-V has no S-mode register that reads back which hart you are - `mhartid` is M-mode - so the id arrives in `a0` once and must be parked per-hart or it is lost; `tp` is where, and it is free because a kernel with no thread-local storage never uses it. Each secondary's stack is a fixed slice of a static arena sized in the source (4 x 32 KiB), so nothing is allocated during bring-up and a hart past the arena is refused rather than given a stack pointer nobody chose. The `satp` write is sound because the root it installs is the kernel's own, built and unchanged, mapping the code doing the write and the stack under it. |
| `arch/riscv64/sv39.rs` | 28 -> 29 (+1) | 2026-09-08: `free_table_tree` becomes `reclaim_user`, which frees a dead task's own pages and tables and leaves the kernel's alone. `U` IS THE DISCRIMINATOR, because on this port nothing else is: x86 reclaims by walking only the low half of its top-level table, which works because its kernel is higher-half, and here the kernel is identity-mapped from zero while a service links at 0x400000, so an index says nothing. `clone_kernel_map` never sets `U` on anything it lends, so a leaf with `U` is the task's and a leaf without it is on loan; a leaf at level 1 is always the kernel's, since `map_page` only ever makes 4 KiB leaves. Every physical address is checked with `phys_in_ram` before it is dereferenced OR freed - a stale entry would otherwise be walked through the identity map and fault the kernel, and freeing one would corrupt the frame bitmap with an address the allocator never issued. The root is deliberately left alone here, because a self-killing task is still executing in that space. |
| `arch/riscv64/mod.rs` | 38 -> 39 (+1) | 2026-09-08: `reclaim_user_frames` and `free_page_table_root` gain their proper halves - the walk, and the last frame. Both refuse a root outside RAM. Returning zero from the reclaim stub was NOT a neutral placeholder: the kill path PRINTS the count, so `freed 0 frames` read as "this task had nothing" when it meant "nothing was reclaimed", and a service faulting in a restart loop leaked its entire address space per cycle until the machine died of `FrameAllocFailed` several hundred restarts away from the cause. Now 84 frames a kill, and 14,651 fault-restart cycles in two minutes with no allocation failure. |
| `arch/riscv64/mod.rs` | 37 -> 38 (+1) | 2026-09-08: the UART's RECEIVE half - `uart_rbr`, the counterpart of `uart_thr` at the same address (a 16550 tells the two apart by the direction of the access, not by the offset). ONE new line, the volatile read, at the width the device tree declares exactly as the transmit side does. Reading it CONSUMES a byte from the FIFO, so it is called only with `LSR_DR` set, and the drain loop around it is bounded by the ring rather than by the FIFO - a UART wedged with `DR` stuck would otherwise spin forever inside a timer interrupt and take the machine with it. The RX RING ITSELF NEEDS NO `unsafe` AT ALL: it is an array of `AtomicU8`, which says what is true (a producer in the tick, a consumer in a syscall) and costs nothing on a machine with the A extension. |
| `arch/riscv64/uaccess.rs` | 0 -> 7 (new file) | 2026-09-07: reading and writing USER memory from the kernel. SEVEN lines: the `sstatus.SUM` set and clear either side of a copy, three `copy_nonoverlapping`s, the `from_raw_parts` over the scratch, and the `time` CSR read. What makes them sound is the WALK, not the copy: every page of a user range is checked against the live page table for presence, the `U` bit and the needed permission BEFORE any byte moves, so a bad pointer is a `false` return rather than a fault - which matters because this port has no kill path yet and a fault would halt. `SUM` is opened by a guard that restores on drop, so no early return can leave the kernel able to touch user memory; it is set for one copy and no longer. The returned slice is over this core's scratch, never over user memory, so nothing downstream can be made to dereference a user address by a later unmap. The pre-walk is a TOCTOU the moment a second hart can unmap underneath it - stated in the module header, and the reason x86's fault-attribution design is the one this adopts when it has a task to kill into. |
| `arch/riscv64/usermode.rs` | 9 -> 17 (+8) | 2026-09-07: the user TASK selftest - user mode and a private address space together, which is the shape a service has. A second `global_asm!` stub, the `copy_nonoverlapping` and `fence`/`fence.i` that make it executable, the context priming, the `switch_context` in and the one OUT OF THE TRAP HANDLER, and the frame frees. The switch out is the notable one: it is called from inside `claim_task_trap` and never returns, abandoning the trap frame on the task's own kernel stack - sound because nothing can resume that task (its context is a static no scheduler knows about) and because the boot context it switches to carries the kernel root, so `satp` is restored on the way out. Sound to free the task's frames afterwards for the same reason. |
| `arch/riscv64/sv39.rs` | 25 -> 28 (+3) | 2026-09-07: `clone_leaf_roots` becomes `clone_kernel_map`, which descends one level where it must. The kernel's identity map and userspace OVERLAP on this port - the kernel is mapped from zero and a service links at 0x400000, so both are inside the FIRST gigapage - so copying a root-level leaf wholesale replaced a service's own text with a kernel mapping that has no `U` bit. It now copies a gigapage only into an EMPTY root slot, leaves a slot the task already owns alone, and where the task holds a pointer table descends and fills only the 2 MiB slots it has not claimed. Same soundness as the rest of the walker (volatile single-PTE accesses, page-aligned frames the kernel owns, indices masked to 9 bits); the added lines are the level-1 read and write and the descent. Permission bits come from the kernel's own leaf, so `U` is never set on anything it copies. |
| `arch/riscv64/sv39.rs` | 17 -> 25 (+8) | 2026-09-07: `clone_leaf_roots` and `free_table_tree` - building a task's address space and taking it back. Same soundness as the rest of the walker: volatile reads and writes of single 64-bit PTE slots in page-aligned frames the kernel owns, indices masked to 9 bits so they are in-frame by construction, addressing identity. The pairing is what makes the frees safe: the clone copies only root-level LEAVES, so anything reachable through a POINTER entry in a task root was built by that task - and `free_table_tree` follows only pointer entries, so it can never walk into the kernel's own tables. `free_table_tree` is `unsafe fn` because it cannot see whether the root is the live `satp` one; freeing the address space you are executing in is the caller's obligation to avoid. |
| `arch/riscv64/context_switch.rs` | 21 -> 30 (+9) | 2026-09-07: the per-task address-space selftest. A third task body, the write of a sentinel into a fresh frame at its physical address (identity-mapped, freshly allocated, so it is the page the mapping points at), the read of that sentinel through the task's OWN address space, the context and root priming, and the reclaim. The private read is guarded by a check that the live root is the expected one, so a switch that failed to install the table reports instead of faulting - a fault there would halt the machine describing a load at an address that means nothing to a reader, burying the actual finding. |
| `arch/riscv64/mod.rs` | 36 -> 37 (+1) | 2026-09-07: `finalize_service_address_space` and `free_page_table_root` gain bodies (net +1). The first is what makes a task switchable AT ALL: the instant `switch_context` writes `satp` the kernel is executing through the new table - the next instruction fetch, the stack under it, the trap vector a fault would need - so a root that maps only the task's own pages is a machine that stops with nothing left able to say why. Both refuse a zero root rather than inventing one. |
| `arch/riscv64/context_switch.rs` | 0 -> 21 (new file) | 2026-09-07: the neutral context-switch surface. FOUR naked functions - `switch_context` itself, the kernel and user first-entry trampolines, and the callee-saved probe - plus the two `unsafe fn` constructors, the two diverging task bodies, the calls between them, and the frame frees. `switch_context` is sound because it touches ONLY the two `TaskContext`s it is handed, at offsets a `const` assertion pins against the struct's size, and because the incoming context's contract (a real return address, a stack the task owns, a valid Sv39 root or zero) is the caller's to meet - which is why it is `unsafe`. The `satp` write is guarded by a comparison against the LIVE register and followed by `sfence.vma`, meeting the SEC-26/27 obligation `arch/CLAUDE.md` places on a weak-ordered port rather than deferring it. The selftest statics are touched only on the boot hart before any other hart exists. |
| `arch/riscv64/mod.rs` | 35 -> 36 (+1) | 2026-09-07: the interrupt-mask seam members gain real bodies and the `context_switch` stub module leaves the file (net +1). `enable_interrupts` / `disable_interrupts` set and clear `sstatus.SIE`; an interrupt raised while it is clear is held pending by hardware rather than lost, so masking defers rather than discards. `local_irq_save` uses `csrrc` to read and clear in ONE instruction - two instructions would leave a window in which an interrupt is taken after the caller believes it has masked. `wait_for_interrupt` is `wfi`, which is architecturally a HINT and only correct inside a re-checking loop, which is how the neutral idle path uses it. Empty bodies here were worse than stubs elsewhere: a `disable_interrupts` that does nothing is a seam member telling the neutral kernel a lie. |
| `arch/riscv64/syscall.rs` | 0 -> 1 (new file) | 2026-09-07: the `ecall` syscall entry. ONE line: the call into the neutral `syscall::dispatch::syscall_handler`, which is `unsafe extern "C"` because it is entered from a privilege transition and must trust nothing it is handed - that is its contract, not a hazard, and it validates every argument itself. The four values come straight out of the trap frame, so they are exactly what the caller placed in its own registers; this file neither dereferences them nor writes a CSR. The x86 and ARM entries call it the same way, for the same reason. |
| `arch/riscv64/usermode.rs` | 0 -> 9 (new file) | 2026-09-07: U-mode entry, and the first code on this ISA that is not the kernel. NINE lines: the `global_asm!` holding the user stub, `enter_user` (naked - saves the callee-saved set and `sp`, latches the kernel stack in `sscratch`, clears `sstatus.SPP` and `sret`s), the `copy_nonoverlapping` of the stub into a fresh frame, a `fence` + `fence.i` so a fetch sees instructions written as data, two `map_in_active_tables` calls, two `sfence.vma` invalidations, and two `free_frame`s of the pages afterwards. The privilege drop is sound because it is a full transfer: nothing after `sret` runs in the kernel until a trap arrives, and the trap arrives on the stack `sscratch` names rather than one U-mode chose. The frames are this function's own, mapped at addresses above every region the kernel maps (the walker refuses to overwrite a live leaf), unmapped before they are freed. The magic syscall and the expected-fault path are gated by `ARMED`, set for exactly one round trip, so neither is reachable from a real task - the ARM32 port learned that one from its own audit. |
| `arch/riscv64/trap.rs` | 6 -> 6 (0) | 2026-09-07: the entry stub gained the `sscratch` kernel-stack swap and saves `sstatus` alongside `sepc`, and `init` now zeroes `sscratch` before installing the vector. Net-neutral on the count - the same three blocks - but the soundness argument is larger and is restated here rather than left at its old scope: a trap from U-mode may not push onto the user's stack, so entry swaps in the latched kernel stack in one `csrrw` and zeroes the latch so a nested trap takes the kernel path instead. `sscratch` has no architecturally specified reset value and the firmware beneath runs on `mscratch`, so it is zeroed rather than assumed - QEMU hands over a zeroed register and would never have shown it. |
| `arch/riscv64/mod.rs` | 34 -> 35 (+1) | 2026-09-07: `map_in_active_tables` gains a body. It walks the root `satp` currently names, maps one page, and issues an address-scoped `sfence.vma` - the fence being required even for a FRESH mapping, because RISC-V permits a negative TLB entry to be cached. It refuses when translation is off rather than inventing a root, which is the only honest answer when there is no active table to add to. |
...
| `arch/riscv64/trap.rs` | 4 -> 6 (+2) | 2026-09-07: the context-saving trap entry and the two interrupt-enable bits. The stub saves all 31 writable registers plus `sepc` on the current stack and restores them before `sret` - all 31 rather than the caller-saved set, because the interrupted code is not a caller and agreed to no convention with the handler. `enable_timer_interrupts` sets `sie.STIE` and `sstatus.SIE`, sound because `stvec` is installed first: enabling before that would give the first tick nowhere to go. |
...
| `arch/riscv64/mod.rs` | 32 -> 33 (+1) | 2026-09-07: `enable_paging` calls `write_page_table_base`. The obligation that makes it sound is discharged immediately above the call: the table maps every address identically from zero to the top of RAM, which includes the instruction stream performing the write and the stack under it. |
...
| File | Change | Why |
|------|--------|-----|
| `arch/x86_64/mod.rs` | 36 -> 37 (+1) | **COM2 presence probe:** `com2_probe` writes and restores the UART scratch register (offset 7) to tell a real 16550 from an absent port. Needed because an absent ISA port reads 0xFF on every register, so the Line Status Register's Data Ready bit reads as SET forever and `com2_try_read_byte` handed `control` an endless stream of phantom 0xFF bytes - measured at 93-100% of core 1, starving the `fs` and `block-driver` placed there. Sound: the scratch register is defined by the 16550 as general storage with NO side effects, the probe restores the prior value, and it runs once at init on this core's own port. |
...
| arch/riscv64/context_switch.rs | 35 | permitted |
| arch/riscv64/usermode.rs | 17 | permitted |
| arch/riscv64/uaccess.rs | 7 | permitted |
| arch/riscv64/display.rs | 6 | permitted |
| arch/riscv64/mod.rs | 67 | permitted |
